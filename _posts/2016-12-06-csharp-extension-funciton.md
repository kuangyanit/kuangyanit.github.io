...
```
这里用List<pre><T><pre>类内置的LastIndexOf方法来求取对象的下标时发现其下标一直为-1（即未在linqDatas集合中找到dataForTest对象），可是该集合中明明包含dataForTest对象的，这是为什么呢？
...
* 自定义类实现IEquatable<pre><T></pre>pre>接口（即实现Equals方法来定义判等规则）
* 编写扩展方法MyLastIndexOf（注意扩展方法的参数，在这里为this IEnumerable<pre><TSource></pre> source与TSource item）
* 对扩展方法添加泛型约束（where TSource:IEquatable<pre><TSource></pre>）
